...
namespace MqttServerTest
{
    public partial class FormClient : Form
    {
        ...
        {
            string topic = txtPubTopic.Text.Trim(); //接受输入的主题，此项没有默认必须输入

            if (string.IsNullOrEmpty(topic)) //判断是否为空 ，为空报错
            {
                MessageBox.Show("发布主题不能为空！");
                return;
            }

            string inputString = txtSendMessage.Text.Trim();  //消息的内容
            int  qos = Convert.ToInt16(qosBox.Text);  //进行QOS的设置！！！ 自己添加代码待优化
            if(qos==0)
            {
                var message = new MqttApplicationMessageBuilder()
             .WithTopic(topic)
             .WithPayload(inputString)
             .WithAtMostOnceQoS()  //QOS为0
             .WithRetainFlag(true)
             .Build();//大致意思就是初始化连接，设置属性。
        ...
        private async Task Subscribe()  //异步编程
        {
        ...
        private void txtSubTopic_TextChanged(object sender, EventArgs e)
        {
